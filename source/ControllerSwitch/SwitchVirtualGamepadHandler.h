...
protected:
    ...
    int32_t m_polling_frequency_ms;
    int32_t m_polling_thread_priority;
    uint32_t m_read_input_timeout_us;
...
public:
    ...
    SwitchVirtualGamepadHandler(std::unique_ptr<IController> &&controller, int32_t polling_frequency_ms, int8_t thread_priority = 0x30);
    ...
    virtual void UpdateInput(uint32_t timeout_us) = 0;
